...
const ALLOWED_ROLES: Role[] = ['qa', 'admin', 'super_admin', 'supervisor']
...
const ALL_STAGES = Array.from(new Set(MOCK_FAILURES.map(f => f.stage)))
...
function FailureHistoryTab() {
  ...
  const filtered = useMemo(() => {
    let rows = MOCK_FAILURES.filter(f => {
      ...
      if (filters.stage !== 'all' && f.stage !== filters.stage) return false
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((f, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-700 whitespace-nowrap">{f.stage}</td>
