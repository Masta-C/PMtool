...
The following skills are available in `~/.claude/skills/`. Invoke them by describing what you want — Claude will apply the right skill.

### Issue tracker

Issues are tracked in GitHub Issues on `Masta-C/PMtool`. See `docs/agents/issue-tracker.md`.
...
Single-context layout — `CONTEXT.md` at repo root for domain language, `docs/adr/` for architecture decisions. See `docs/agents/domain.md`.

### Available skills

| Skill | When to use |
|---|---|
| `grill-me` | Stress-test a plan or design — relentless one-question-at-a-time interview |
| `to-prd` | Turn a conversation or requirements into a PRD, saved to the repo |
| `to-issues` | Break a PRD or plan into GitHub issues |
| `triage` | Triage incoming GitHub issues through the label state machine |
| `tdd` | Write the test first, then implement — red-green-refactor loop |
| `diagnose` | Structured debugging loop for hard bugs or regressions |
| `improve-codebase-architecture` | Find architectural improvements informed by domain language |
| `zoom-out` | Step back and get a higher-level perspective on the current work |
| `prototype` | Build a throwaway prototype to validate a design before committing |
| `handoff` | Compact the current session into a handoff doc for another agent |
| `pmtool-dev-ready` | Start, seed, and validate the full local dev environment |
| `pmtool-grill-me` | Project-specific grill-me |
...
- **Firestore offline persistence**: `enableIndexedDbPersistence` enabled in `client.ts` — PWA offline support active
- **PR template**: `.github/PULL_REQUEST_TEMPLATE.md` created
- **`.env.example`**: Created for developer onboarding
- **Storage rules**: RBAC implemented — products (admin+), work order attachments (all roles), user avatars (own + admin)
